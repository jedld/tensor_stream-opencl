...
__kernel void conv2d_backprop_input(const int height, const int width, __global const <%= ctype %> *filter, __global const <%= ctype %> *grad, __global <%= ctype %> *output) {
    ...
    const int h_index = get_global_id(1); // orig image y
    const int w_index = get_global_id(2); // orig image x
    ...
    const int image_index = batch_index * grad_height * grad_width * <%= out_ch %>;
        ...
        for(int y = 0; y < <%= fh %>; y++) {
          for (int x = 0; x < <%= fw %>; x++) {
            if (y <= h_index && x <= w_index && ( (h_index - y) % <%= stride[0]%> == 0) && ( (w_index - x) % <%= stride[1]%> == 0)) {              
              g += filter[y*<%= fw * ch * out_ch %> + x*<%= ch * out_ch %> + (channel_index*<%= out_ch %>) + out_channel_index];
            } 
          }
        }
    }
