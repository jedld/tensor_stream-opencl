...
selection = tf.placeholder(:int32)
...
test_data = { x => test_x, y_ => test_y, pkeep => 1.0 }

selections = (0...60_000).to_a

(0..10001).each do |i|
  # load batch of images and correct answers
  current_selection = selections.sample(100)
  train_data = { x => train_x, y_ => train_y, selection => current_selection, step => i, pkeep => 0.75 }
  ...
  if (i % 10 == 0)
    ...
    a_train, c_train, l = sess.run([accuracy, cross_entropy, lr], feed_dict: { x => train_x, y_ => train_y, selection => current_selection, step => i, pkeep => 1.0})
